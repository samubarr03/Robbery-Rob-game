...
Matrix::Matrix()
{
    std::string filename = "resources/mapstep8.txt";
    ...
    loadTexture(TYPE_LOOT, "loot.png", 2, 2);
    ...
    loadTexture(TYPE_EXIT, "exit.png", 3, 3);
}
...
void Matrix::drawItemInPosition(sf::Vector2f mousePos, int selectedTool)
{
    ...
    if (cellX + w > GRID_SIZE || cellY + h > GRID_SIZE)
    {
        ...
        if (selectedTool != TYPE_FLOOR)
    {
        for (int dx = 0; dx < w; dx++)
        {
            for (int dy = 0; dy < h; dy++)
            {
                if (map[cellX + dx][cellY + dy] != TYPE_FLOOR)
                {
                    std::cerr << "Errore: c'è già un oggetto in questa area.\n";
                    return;
                }
            }
        }
...
void Matrix::calculateTotalScore()
{
    score = 0;
