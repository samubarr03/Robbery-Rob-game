...
bool Guard::canMoveTo(sf::Vector2i newPos) const
{
    if (newPos.x < 0 || newPos.y < 0 ||
        ...
        return false;
    auto &map = matrix.getMap();
    return (map[newPos.x][newPos.y] == TYPE_FLOOR || map[newPos.x][newPos.y] == TYPE_GUARD_ROUTE) &&
           (map[newPos.x][newPos.y + 1] == TYPE_FLOOR || map[newPos.x][newPos.y + 1] == TYPE_GUARD_ROUTE) &&
           (map[newPos.x + 1][newPos.y] == TYPE_FLOOR || map[newPos.x + 1][newPos.y] == TYPE_GUARD_ROUTE) &&
           (map[newPos.x + 1][newPos.y + 1] == TYPE_FLOOR || map[newPos.x + 1][newPos.y + 1] == TYPE_GUARD_ROUTE);
}
